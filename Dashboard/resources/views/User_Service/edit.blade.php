...
<div class="form-container">
    <form action="{{ route('User_Service.update', $services->id) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="mb-3">
            <label for="IdAct" class="form-label">ID Actif</label>
            <input type="text" class="form-control" id="IdAct" name="IdAct" value="{{ $services->IdAct }}" required>
        </div>
        <div class="mb-3">
            <label for="FormatData" class="form-label">Format</label>
            <input type="text" class="form-control" id="FormatData" name="FormatData" value="{{ $services->FormatData }}" required>
        </div>
        <div class="mb-3">
            <label for="SourceData" class="form-label">Source</label>
            <input type="text" class="form-control" id="SourceData" name="SourceData" value="{{ $services->SourceData }}" required>
        </div>
