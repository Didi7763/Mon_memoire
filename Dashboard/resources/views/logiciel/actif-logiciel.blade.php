...
<div class="p-8 overflow-y-auto" style="max-height: calc(100vh - 64px);">
    ...
    <div id="main-content" class="relative" style="min-height: 75vh; display: none; opacity: 0;">
        <div class="flex justify-between items-center mb-8 p-4 bg-white rounded-lg shadow-sm min-h-[6rem]">
            <h2 class="text-xl font-bold">Liste des Actifs Logiciels</h2>
            <a href="{{ route('logiciel.create') }}" class="flex items-center gap-2 px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors">
            ...
            <table class="w-full text-xs">
                <thead>
                    <tr class="border-b">
                        ...
                        <th class="text-left p-3">Clé</th>
                        <th class="text-left p-3">Date d'installation</th>
                        <th class="text-left p-3">Date d'expiration</th>
                ...
                <tbody>
                    ...
                    <tr class="border-b hover:bg-gray-50">
                        <td class="p-3">{{ $logiciel->actif->IdAct }}</td>
                        <td class="p-3">{{ $logiciel->actif->NomAct }}</td>
                        <td class="p-3">{{ $logiciel->VersionLog }}</td>
                        <td class="p-3">{{ $logiciel->TypLicLog }}</td>
                        <td class="p-3">{{ $logiciel->NbrLicLog }}</td>
                        <td class="p-3">{{ $logiciel->CleLicLog }}</td>
                        <td class="p-3">{{ \Carbon\Carbon::parse($logiciel->DatAchLog)->format('d/m/Y') }}</td>
                        <td class="p-3">{{ \Carbon\Carbon::parse($logiciel->DatExpLog)->format('d/m/Y') }}</td>
                        <td class="p-3">{{ $logiciel->actif->ComtAct }}</td>
                        <td class="p-3">
                            <div class="flex gap-2">
                                <!-- Bouton "Modifier" plus petit -->
                                <a href="{{ route('logiciel.edit', $logiciel->id) }}" class="px-2 py-1 bg-yellow-500 text-white rounded-lg text-xs hover:bg-yellow-600 transition-colors">Modifier</a>
                                <!-- Bouton "Supprimer" plus petit -->
                                <form action="{{ route('logiciel.destroy', $logiciel->id) }}" method="POST" onsubmit="return confirm('Confirmer la suppression ?')">
                                    @csrf
                                    @method('DELETE')
                    ...
                    @empty
                    <tr>
                        <td colspan="10" class="p-3 text-center">Aucun actif logiciel trouvé</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
